...
namespace gazebo
{
    class WestwoodDrawForcePlugin : public VisualPlugin
    {
        public:
        WestwoodDrawForcePlugin():line(NULL){}
        ~WestwoodDrawForcePlugin(){
        ...
        void Load(rendering::VisualPtr _parent, sdf::ElementPtr _sdf )
        {
            ...
            this->force_sub = this->rosnode->subscribe(this->topic_name+"/"+"the_force", 30, &WestwoodDrawForcePlugin::GetForceCallback, this);
            this->update_connection = event::Events::ConnectPreRender(boost::bind(&WestwoodDrawForcePlugin::OnUpdate, this));
    ...
    GZ_REGISTER_VISUAL_PLUGIN(WestwoodDrawForcePlugin)
}
