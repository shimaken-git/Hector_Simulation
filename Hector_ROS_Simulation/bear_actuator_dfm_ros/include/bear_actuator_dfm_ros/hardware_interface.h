...
class HardwareInterface : public hardware_interface::RobotHW
{
 ...
 private:
  ...
  std::map<uint16_t, uint16_t> a2jMap;   // actuator to joint map use Kp,Kd setting
  std::vector<bearJoint> joints_;
  std::vector<uint8_t> idList;
  std::vector<uint16_t> gimIdList;
  std::vector<uint8_t> write_add;
  std::vector<uint8_t> write_add2;
  std::vector<uint8_t> read_add;
  std::vector<uint8_t> config_add;
